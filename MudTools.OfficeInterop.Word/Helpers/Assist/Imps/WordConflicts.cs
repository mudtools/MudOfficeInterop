...
internal class WordConflicts : IWordConflicts
{
    ...
    /// <summary>
    /// 初始化 <see cref="WordConflicts"/> 类的新实例。
    /// </summary>
    /// <param name="conflicts">要封装的原始 COM Conflicts 对象。</param>
    internal WordConflicts(MsWord.Conflicts conflicts)
    {
    ...
    public IWordConflict? this[int index]
    {
        get
        {
            if (_conflicts == null || index < 1 || index > Count) return null;
            try
            {
    ...
    /// <summary>
    /// 释放由 <see cref="WordConflicts"/> 使用的非托管资源，并选择性地释放托管资源。
    /// </summary>
    ...
    /// <summary>
    /// 释放由 <see cref="WordConflicts"/> 使用的所有资源。
    /// </summary>
    public void Dispose()
    {
        Dispose(true);
    ...
    #region IEnumerable<IWordConflict> 实现

    /// <inheritdoc/>
    public IEnumerator<IWordConflict> GetEnumerator()
    {
