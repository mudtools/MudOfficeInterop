...
// <summary>
/// Word.Characters 的封装实现类。
/// </summary>
internal class WordCharacters : IWordCharacters
{
    private MsWord.Characters _characters;
    private bool _disposedValue;

    internal WordCharacters(MsWord.Characters characters)
    {
    ...
    /// <inheritdoc/>
    public IWordApplication Application => _characters != null ? new WordApplication(_characters.Application) : null;
    ...
    /// <inheritdoc/>
    public IWordRange First => _characters?.First != null ? new WordRange(_characters.First) : null;

    /// <inheritdoc/>
    public IWordRange Last => _characters?.Last != null ? new WordRange(_characters.Last) : null;
    ...
    public IWordRange this[int index]
    {
        get
        {
            ...
            try
            {
                var comRange = _characters[index];
    ...
    public IWordRange GetRange(int start, int end)
    {
        ...
        try
        {
            // 在 Word 中，Characters 集合的范围操作需要通过父 Range 来实现
            var parentRange = _characters.Parent as MsWord.Range;
            if (parentRange != null)
            {
                var newRange = parentRange.Duplicate;
                newRange.Start = parentRange.Start + validStart - 1;
                newRange.End = parentRange.Start + validEnd;
    ...
    /// <inheritdoc/>
    public IWordRange Find(string findText, bool forward = true, MsWord.WdFindWrap wrap = MsWord.WdFindWrap.wdFindStop,
    ...
    {
        if (_characters == null || string.IsNullOrEmpty(findText)) return null;

        try
        {
            MsWord.Range? parentRange = _characters.Parent as MsWord.Range;
            if (parentRange != null)
            {
    ...
    public int Replace(string findText, string replaceText, bool replaceAll = false,
                      ...
                      bool matchSoundsLike = false, bool matchAllWordForms = false)
    {
        if (_characters == null || string.IsNullOrEmpty(findText)) return 0;
    ...
    {
        if (_characters == null || string.IsNullOrEmpty(text)) return null;
    ...
    {
        if (_characters == null || string.IsNullOrEmpty(text) || index < 1 || index > Count + 1) return null;
        ...
        try
        {
            ...
            {
                // 计算插入位置
                int insertPosition = parentRange.Start + index - 1;
    ...
    {
        if (_characters == null || start < 1 || count <= 0) return;
        ...
        try
        {
            ...
            if (parentRange != null)
            {
                int deleteStart = parentRange.Start + start - 1;
                int deleteEnd = Math.Min(deleteStart + count, parentRange.End);

                var deleteRange = parentRange.Duplicate;
                deleteRange.SetRange(deleteStart, deleteEnd);
                deleteRange.Delete();
            }
        }
        catch
