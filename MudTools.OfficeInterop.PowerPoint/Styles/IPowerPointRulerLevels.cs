...
public interface IPowerPointRulerLevels : IOfficeObject<IPowerPointRulerLevels, MsPowerPoint.RulerLevels>, IEnumerable<IPowerPointRulerLevel?>, IDisposable
{
    ...
    /// 获取集合中的标尺层级数量。
    /// </summary>
    /// <value>集合中的标尺层级数量。</value>
    int Count { get; }

    /// <summary>
    /// 获取创建此标尺层级集合的 PowerPoint 应用程序实例。
    /// </summary>
    ...
    /// 获取此标尺层级集合的父对象。
    /// </summary>
    /// <value>表示此标尺层级集合父对象的 <see cref="object"/>。</value>
    object? Parent { get; }

    /// <summary>
    /// 通过索引获取集合中的指定标尺层级。
    /// </summary>
    /// <param name="index">要获取的标尺层级的索引（从1开始）。</param>
    /// <value>位于指定索引处的 <see cref="IPowerPointRulerLevel"/> 对象。</value>
    IPowerPointRulerLevel? this[int index] { get; }
}