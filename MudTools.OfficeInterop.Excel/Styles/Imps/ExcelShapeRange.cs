...
internal class ExcelShapeRange : IExcelShapeRange
{
    ...
    public ShapeRangeStatistics GetStatistics()
    {
        var stats = new ShapeRangeStatistics
        {
            TotalCount = Count,
            VisibleCount = 0,
            HiddenCount = 0,
        ...
        var types = new HashSet<MsoShapeType>();
    ...
    {
        if (_shapeRange == null || Count == 0)
            return new TypeStatistics[0];

        var typeCount = new Dictionary<MsoShapeType, int>();
    ...
    public ShapeSizeDistribution GetSizeDistribution()
    {
        var distribution = new ShapeSizeDistribution
        {
            SmallShapes = 0,
            MediumShapes = 0,
            LargeShapes = 0,
        ...
        for (int i = 1; i <= Count; i++)
        {
            try
            {
                ...
                if (shape != null)
                {
                    ...
                    if (area < 2500) // 50x50
                        distribution.SmallShapes++;
                    else if (area < 40000) // 200x200
                        distribution.MediumShapes++;
                    else if (area < 250000) // 500x500
                        distribution.LargeShapes++;
                    else
                        distribution.ExtraLargeShapes++;
                }
            }
            catch
            {
