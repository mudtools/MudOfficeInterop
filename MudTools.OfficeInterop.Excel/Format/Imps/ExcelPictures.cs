...
internal class ExcelPictures : IExcelPictures
{
    ...
    public int AddRange(PictureData[] picturesData)
    {
        if (_pictures == null || picturesData == null || picturesData.Length == 0)
            return 0;

        int successCount = 0;
        foreach (var data in picturesData)
        {
            if (Add(data.Filename, null) != null)
                successCount++;
        }
        return successCount;
    }
    ...
    public void ArrangeInGrid(int rows, int columns, double horizontalSpacing = 10, double verticalSpacing = 10)
    {
        if (_pictures == null || Count == 0 || rows <= 0 || columns <= 0)
            return;
        ...
        try
        {
            ...
            for (int row = 0; row < rows && pictureIndex <= Count; row++)
            {
                for (int col = 0; col < columns && pictureIndex <= Count; col++)
                {
                    try
                    {
                        var picture = this[pictureIndex];
                        if (picture != null)
                        {
                            picture.Left = baseLeft + col * (picture.Width + horizontalSpacing);
                            picture.Top = baseTop + row * (picture.Height + verticalSpacing);
                        }
                        pictureIndex++;
                    }
                    catch
    ...
    public PictureInfo[] GetAllPictureInfo()
    {
        if (_pictures == null || Count == 0)
            return new PictureInfo[0];
    ...
    public PictureStatistics GetStatistics()
    {
        var stats = new PictureStatistics
        {
            TotalCount = Count,
            VisibleCount = 0,
            HiddenCount = 0,
    ...
    public PictureSizeDistribution GetSizeDistribution()
    {
        var distribution = new PictureSizeDistribution
        {
            SmallPictures = 0,
            MediumPictures = 0,
            LargePictures = 0,
            ExtraLargePictures = 0
        };
        ...
        for (int i = 1; i <= Count; i++)
        {
            try
            {
                ...
                if (picture != null)
                {
                    double area = picture.Width * picture.Height;

                    if (area < 10000) // 100x100
                        distribution.SmallPictures++;
                    else if (area < 250000) // 500x500
                        distribution.MediumPictures++;
                    else if (area < 1000000) // 1000x1000
                        distribution.LargePictures++;
                    else
                        distribution.ExtraLargePictures++;
                }
            }
            catch
            {
