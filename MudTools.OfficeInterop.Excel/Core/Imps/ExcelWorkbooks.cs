...
internal class ExcelWorkbooks : IExcelWorkbooks
{
    ...
    public WorkbookInfo[] GetAllWorkbookInfo()
    {
        if (_workbooks == null || Count == 0)
            return [];

        List<WorkbookInfo> result = [];
        for (int i = 1; i <= Count; i++)
        {
            try
            {
                ...
                if (workbook != null)
                {
                    var info = new WorkbookInfo
                    {
                        ...
                        FileFormat = Path.GetExtension(workbook.FullName ?? "").TrimStart('.'),
                    };
                    result.Add(info);
                }
            }
    ...
    public WorkbookStatistics GetStatistics()
    {
        var stats = new WorkbookStatistics
        {
            TotalCount = Count,
            SavedCount = 0,
            UnsavedCount = 0,
            ProtectedCount = 0,
            ReadOnlyCount = 0,
            AverageWorksheetCount = 0,
            MaxWorksheetCount = 0,
            MinWorksheetCount = int.MaxValue,
            AverageFileSize = 0,
            MaxFileSize = 0,
            MinFileSize = double.MaxValue
        };
        ...
        for (int i = 1; i <= Count; i++)
        {
            try
            {
                ...
                if (workbook != null)
                {
                    ...
                    int worksheetCount = workbook.WorksheetCount;
                    totalWorksheets += worksheetCount;

                    if (worksheetCount > stats.MaxWorksheetCount)
                        stats.MaxWorksheetCount = worksheetCount;

                    if (worksheetCount < stats.MinWorksheetCount)
                        stats.MinWorksheetCount = worksheetCount;

                    double fileSize = workbook.FileSize;
                    totalFileSize += fileSize;

                    if (fileSize > stats.MaxFileSize)
                        stats.MaxFileSize = fileSize;

                    if (fileSize < stats.MinFileSize)
                        stats.MinFileSize = fileSize;
                }
            }
            catch
            {
        ...
        stats.MinWorksheetCount = stats.MinWorksheetCount == int.MaxValue ? 0 : stats.MinWorksheetCount;
        stats.MinFileSize = stats.MinFileSize == double.MaxValue ? 0 : stats.MinFileSize;
        stats.AverageWorksheetCount = Count > 0 ? (double)totalWorksheets / Count : 0;
        stats.AverageFileSize = Count > 0 ? totalFileSize / Count / (1024 * 1024) : 0; // 转换为MB
        stats.MaxFileSize = stats.MaxFileSize / (1024 * 1024); // 转换为MB
        stats.MinFileSize = stats.MinFileSize / (1024 * 1024); // 转换为MB
    ...
    {
        if (_workbooks == null || Count == 0)
            return new FileTypeStatistics[0];

        var formatCount = new System.Collections.Generic.Dictionary<string, int>();
        ...
        for (int i = 1; i <= Count; i++)
        {
            try
            {
                ...
                if (workbook != null)
                {
                    string format = System.IO.Path.GetExtension(workbook.FullName ?? "").ToLower().TrimStart('.');
                    if (string.IsNullOrEmpty(format))
                        format = "unknown";

                    if (formatCount.ContainsKey(format))
                        formatCount[format]++;
                    else
                        formatCount[format] = 1;
                }
            }
            catch
            {
        ...
        var result = new System.Collections.Generic.List<FileTypeStatistics>();
        int totalCount = Count;

        foreach (var kvp in formatCount)
        {
            result.Add(new FileTypeStatistics
            {
                FileFormat = kvp.Key,
                Count = kvp.Value,
    ...
    public SizeDistribution GetSizeDistribution()
    {
        ...
        for (int i = 1; i <= Count; i++)
        {
            try
            {
                ...
                if (workbook != null)
                {
                    double fileSizeMB = workbook.FileSize / (1024.0 * 1024.0);

                    if (fileSizeMB < 1)
                        distribution.SmallFiles++;
                    else if (fileSizeMB < 10)
                        distribution.MediumFiles++;
                    else if (fileSizeMB < 100)
                        distribution.LargeFiles++;
                    else
                        distribution.ExtraLargeFiles++;
                }
            }
            catch
            {
